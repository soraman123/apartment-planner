...
const { data: { subscription } } = supabase.auth.onAuthStateChange((_e, session) => { setSession(session); setLoading(false); });
...
const loadAll = async (currentSession) => {
  setLoading(true);
  const uid = currentSession.user.id;

  const { data: prof } = await supabase.from("profiles").select("*").eq("id", uid).single();
  setProfile(prof);
  if (!prof?.onboarded) { await completeOnboarding({ spaceName: "My Apartment", selectedSections: SEGMENTS.map(s => s.id) }); setLoading(false); return; }
...
<input defaultValue={sectionForm.label} onBlur ={e => setSectionForm(f => ({ ...f, label: e.target.value }))} placeholder="Section name *" style={{ ...inputStyle, maxWidth: 200 }} />
<input defaultValue={sectionForm.sub} onBlur ={e => setSectionForm(f => ({ ...f, sub: e.target.value }))} placeholder="Subtitle (optional)" style={{ ...inputStyle, maxWidth: 240 }} />
...
<div style={{ gridColumn: "1 / -1" }}><label style={labelStyle}>Title *</label><input defaultValue={form.title} onBlur={e => setForm(f => ({ ...f, title: e.target.value }))} placeholder="e.g. Linen sofa" style={inputStyle} /></div>
<div style={{ gridColumn: "1 / -1" }}><label style={labelStyle}>Notes</label><textarea defaultValue={form.note} onBlur={e => setForm(f => ({ ...f, note: e.target.value }))} rows={3} style={{ ...inputStyle, resize: "vertical" }} /></div>
<div><label style={labelStyle}>Cost ($)</label><input defaultValue={form.cost} onBlur={e => setForm(f => ({ ...f, cost: e.target.value }))} placeholder="0.00" type="number" min="0" step="0.01" style={inputStyle} /></div>
<div><label style={labelStyle}>Link</label><input defaultValue={form.link} onBlur={e => setForm(f => ({ ...f, link: e.target.value }))} placeholder="https://..." style={inputStyle} /></div>
